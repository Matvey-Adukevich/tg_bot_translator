    ...
    while True:
        try:
            ...
            def translation(message):
                if message.text == '/menu':
                    return menu(message)
                if os.path.isfile('000001.jpg'):
                    os.remove('000001.jpg')
                elif os.path.isfile('000001.png'):
                    os.remove('000001.png')
                elif os.path.isfile('000001.jpeg'):
                    os.remove('000001.jpeg')
                ...
                wordtofind = message.text.lower()
                gel = session3.get(f'https://www.linguatools.de/kollokationen-en/bolls/?utf8=%E2%9C%93&lemmahits=100&query={wordtofind}&commit=Search+Collocations%21')
                collocations = parser_collocations(wordtofind, gel)
                col = [f'<code>{i}</code>' for i in collocations[:15]]
                sentences = parser_sentences(wordtofind, gel)
                sent = [i for i in sentences[:5]]
                ...
                if a!=[]:
                    ...
                    #     picturename = '000001.jpg'
                    # except:
                    #     img = open('000001.png', 'rb')
                    #     picturename = '000001.png'
                    if os.path.isfile('000001.jpg'):
                        img = open('000001.jpg', 'rb')
                        # picturename = '000001.jpg'
                        ...
                        img = open('000001.jpeg', 'rb')
                        bot.send_photo(message.chat.id, img)
                    # img = open(picturename, 'rb')
                    # bot.send_photo(message.chat.id, img)
                    # os.remove(f'C://Users//matve//python_projects//bot translator 2023-24//{picturename}')
                    # audioname = f'{message.text}_audio.mp3'
                    # print(audioname)
