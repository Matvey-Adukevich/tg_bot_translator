...
def programm():
    while True:
        try:
            ...
            URL_3 = 'https://dictionary.cambridge.org/ru/thesaurus/'
            ...
            def ant_syn_word():
                print('7')
                if page_number == 1:
                    pgwrd = f'https://dictionary.cambridge.org/ru/thesaurus/{word_A1}'
                    pgwrd = ''.join(pgwrd.split())
                    page_word = session12.get(pgwrd)
                if page_number == 2:
                    pgwrd = f'https://dictionary.cambridge.org/ru/thesaurus/{word_A2}'
                    pgwrd = ''.join(pgwrd.split())
                    page_word = session12.get(pgwrd)
                if page_number == 3:
                    pgwrd = f'https://dictionary.cambridge.org/ru/thesaurus/{word_B1}'
                    pgwrd = ''.join(pgwrd.split())
                    page_word = session12.get(pgwrd)
                if page_number == 4:
                    pgwrd = f'https://dictionary.cambridge.org/ru/thesaurus/{word_B2}'
                    pgwrd = ''.join(pgwrd.split())
                    page_word = session12.get(pgwrd)
                if page_number == 5:
                    pgwrd = f'https://dictionary.cambridge.org/ru/thesaurus/{word_C1}'
                    pgwrd = ''.join(pgwrd.split())
                    page_word = session12.get(pgwrd)
                if page_number == 6:
                    pgwrd = f'https://dictionary.cambridge.org/ru/thesaurus/{word_C2}'
                    pgwrd = ''.join(pgwrd.split())
                    page_word = session12.get(pgwrd)
                print('9')
                print(page_word)
                soup = b(page_word.text, 'html.parser')
                synonyms_1 = soup.find_all(class_='dx-h dthesButton synonym')
                # synonyms_2 = soup.find_all(class_='KmScG4NplKj_3H5E3oA_')
                h = list(set(e.text for e in synonyms_1))
                ...
                #     if i not in h:
                #         h.append(i)
                antonyms_1 = soup.find_all(class_='dx-h dthesButton opposite')
                # antonyms_2 = soup.find_all(class_='SynonymAndAntonymCard-module_s10_c00eb935831f2713d47f SynonymAndAntonymCard-module_antonym_d70dabe8d34f0b3020a3')
                c = list(set(d.text for d in antonyms_1))
                print(c)
            ...
            def parser_pictures(word_to_find):
                session6 = HTMLSession()
                response = session6.get(f'https://www.google.com/search?sca_esv=600400644&q={word_to_find.lower()}&tbm=isch&source=lnms&sa=X&ved=2ahUKEwjtxb_A5PGDAxWKDxAIHcGlAAMQ0pQJegQIDBAB&biw=1350&bih=598&dpr=1')
                ...
                session7 = HTMLSession()
                p = session7.get(images)
                out = open(f".//pictures//{word_to_find.lower()}.png", "wb")
                # out.save(f"{word_to_find}.png", 'png', quality=100)
                out.write(p.content)
                # out.save(f"{word_to_find}.eps", 'eps', quality=100)
                out.close()
            ...
            def ant_to_translate(m):
                soup_2 = b(m.text, 'html.parser')
                antonyms_1 = soup_2.find_all(class_='dx-h dthesButton opposite')
                global ant_array
                ant_array = list(set(d.text for d in antonyms_1))
                print(ant_array)
                return ant_array
            ...
            def translation_threads(mes):
                ...
                picpic.start()
                klg = f'https://dictionary.cambridge.org/ru/thesaurus/{str(mes).lower()}'
                print(mes)
                klg = ''.join(klg.split())
                m = session13.get(klg)
                antant = Thread(target=ant_to_translate,args=(m,))
                synsyn = Thread(target=syn_to_translate, args=(m,))
            ...
            def translation(message):
                ...
                translation_1 = soup_1.find_all('div', class_='NZKOFkdkcvYgD3lqOIJw')
                ...
                if a!=[]:
                    ...
                    # word_to_translate = str(message.text).lower()
                    ...
                    print(sent)
                    if a!=[]:
                        img = open(f'.//pictures//{message.text.lower()}.png', 'rb')
                        print('picture!')
            ...
            def only_synonym(message):
                ...
                r = session18.head(f'https://www.dictionary.com/browse/{message.text.lower()}')
                status = int(r.status_code)
                print(status)
                if status == 200:
                    word = Word2Vec.load('wv')
                    try:
                        word10 = word.wv.most_similar(message.text.lower(), topn=1)
                        bot.send_message(message.chat.id,text=word10[0][0])
                        tgr = True
                    except:
                        tgr = False
                    print(tgr)
                    if tgr == True:
                        if language == 'eng':
                            msg4 = bot.send_message(message.chat.id, text="Enter word: ")
                        elif language == 'ru':
                            msg4 = bot.send_message(message.chat.id, text="Введите слово: ")
                        bot.register_next_step_handler(msg4, only_synonym)
                if (status!=200 or tgr==False):
                    if language == 'eng':
                        msg4 = bot.send_message(message.chat.id, text="Sorry, we didn't find any synonyms for this word! Please, check your word and try again")
                    elif language == 'ru':
                        msg4 = bot.send_message(message.chat.id, text="Извините, мы не нашли синонимов к данному слову! Пожалуйста, проверьте правильность написания слова и попробуйте заново")
                    bot.register_next_step_handler(msg4, only_synonym)
            ...
            def menu(message):
                ...
                btn4 = types.KeyboardButton("Find Synonym for word(Найти синоним)")
            ...
            def func(message):
                ...
                elif message.text=='Find Synonym for word(Найти синоним)':
                    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
                    menu = types.KeyboardButton("/menu")
                    markup.add(menu)
                    if language == 'eng':
                        bot.send_message(message.chat.id, text="Enter '/menu', if you want go to the menu", reply_markup=markup)
                        msg10 = bot.send_message(message.chat.id, text="Enter the word for which You want to find synonym")
                    elif language == 'ru':
                        bot.send_message(message.chat.id, text="Введите '/menu', если хотите вернуться в меню.",reply_markup=markup)
                        msg10 = bot.send_message(message.chat.id, text="Введите слово, для которого Вы хотите найти синоним")
                    '''прописать функцию по проверке слова + вывод синонима'''
                    bot.register_next_step_handler(msg10, only_synonym)
